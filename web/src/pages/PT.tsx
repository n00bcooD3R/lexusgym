...
export default function PTPage() {
  ...
  const loadPTMembers = useCallback(async () => {
    try {
      ...
      console.error("Failed to load PT members:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (selected) {
    return <PTClientDetail member={selected} onBack={() => { setSelected(null); loadPTMembers(); }} />;
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem" }}>
        ...
        <Link to="/members" className="btn btn-ghost" style={{ fontSize: "0.875rem", padding: "0.55rem 1rem", minHeight: 38 }}>All Members</Link>
      </div>
            ...
            return (
              <button key={m.id} onClick={() => setSelected(m)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.85rem 1rem" }}>
                  {m.photo_url
                    ? <img src={m.photo_url} alt={m.name} style={{ width: 40, height: 40, borderRadius: "50%", objectFit: "cover", border: "2px solid var(--accent)" }} />
                    : <div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,var(--accent),var(--accent2))", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "white" }}>{m.name.charAt(0).toUpperCase()}</div>}
...
function PTClientDetail({ member, onBack }: { member: any; onBack: () => void }) {
  ...
  useEffect(() => {
    supabase.from("client_tokens").select("token").eq("member_id", member.id).single()
      .then(({ data }) => { if (data) setToken(data.token); });
  }, [member.id]);

  const generateToken = async () => {
    setGenLoading(true);
    const { data, error } = await supabase.from("client_tokens")
      .upsert({ member_id: member.id }, { onConflict: "member_id" })
      .select("token").single();
    if (data) setToken(data.token);
    setGenLoading(false);
    if (error) {
      alert("Error generating portal token: " + error.message);
    }
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flex: 1, minWidth: "200px" }}>
          {member.photo_url
            ? <img src={member.photo_url} alt={member.name} style={{ width: 44, height: 44, borderRadius: "50%", objectFit: "cover", border: "2px solid var(--accent)" }} />
            : <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg,var(--accent),var(--accent2))", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "white", fontSize: "1.1rem" }}>{member.name.charAt(0).toUpperCase()}</div>}
          <div>
        ...
        <Link to={`/members/${member.id}`} className="btn btn-ghost" style={{ fontSize: "0.8rem", padding: "0.4rem 0.8rem", minHeight: 34 }}>View Profile</Link>
      </div>
          ...
          {token ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                ...
                <a href={clientUrl!} target="_blank" rel="noreferrer" className="btn btn-ghost" style={{ fontSize: "0.875rem", padding: "0.5rem 1rem", minHeight: 36 }}>
                  🔍 Preview
                </a>
            ...
            <button onClick={generateToken} className="btn btn-primary" disabled={genLoading} style={{ alignSelf: "flex-start", minHeight: 38 }}>
...
function WorkoutEditor({ memberId }: { memberId: string }) {
  ...
  const loadWorkoutPlan = useCallback(async () => {
    const { data } = await supabase.from("workout_plans").select("*").eq("member_id", memberId).order("day_number");
    const existing = data ?? [];
  ...
  const saveDay = async (day: WorkoutDay) => {
    ...
    const { error } = day.id
      ? await supabase.from("workout_plans").update(payload).eq("id", day.id)
      : await supabase.from("workout_plans").upsert({ ...payload }, { onConflict: "member_id,day_number" }).select().single();
    setSaving(false);
    if (!error) {
...
function DietEditor({ memberId }: { memberId: string }) {
  ...
  const loadDietPlan = useCallback(async () => {
    const { data } = await supabase.from("diet_plans").select("*").eq("member_id", memberId).order("meal_order");
  ...
  const saveMeal = async (meal: DietMeal, idx: number) => {
    ...
    const { data, error } = meal.id
      ? await supabase.from("diet_plans").update(payload).eq("id", meal.id).select().single()
      : await supabase.from("diet_plans").insert(payload).select().single();
    if (!error && data) {
      setMeals(m => m.map((x, i) => i === idx ? { ...x, id: data.id } : x));
    }
    setSaving(null);
    if (!error) {
  ...
  const deleteMeal = async (meal: DietMeal, idx: number) => {
    if (meal.id) await supabase.from("diet_plans").delete().eq("id", meal.id);
